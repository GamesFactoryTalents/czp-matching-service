...
{{
  ...
  "recruiter_brief": "<For Shortlist/Review only: 3-5 sentences written directly to the recruiter. Must cover these points where data is available: (1) SENIORITY — state the candidate's seniority level versus what the job requires and whether it matches or not; (2) LOCATION — if the candidate's location differs from the job location, explicitly state both locations and whether the candidate is open to relocation (if relocation data is missing, flag it as unknown); if locations match, confirm it briefly; (3) SALARY — if BOTH the job salary range AND candidate expected salary are provided, state both figures and whether they align or show a gap; if either is missing, skip salary entirely — do not mention it; (4) add 1-2 sentences on the strongest reason to interview and the key thing to verify. For Pass candidates: empty string.>"
}}
