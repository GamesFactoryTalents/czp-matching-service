...
{{
  ...
  "recruiter_brief": "<For Shortlist/Review only: 4-6 sentences written directly to the recruiter. Write it as a flowing narrative — do not use bullet points or section headers. Cover these topics in this exact order: (1) SENIORITY — state the candidate's self-reported seniority level and whether it matches, exceeds, or falls short of what the job requires; if the CV evidence confirms or contradicts the self-reported level, note that briefly; (2) SKILLS AND SPECIALITIES — name the specific skills, stack, and specialisations that align with the job requirements, then identify the primary gap or thing to verify; (3) RELEVANT EXPERIENCE — highlight the most relevant experience from the CV that directly matches the job requirements — be concrete, cite roles, projects, or shipped titles; (4) SALARY — only include this if BOTH the job salary range AND the candidate's expected salary are provided; state both figures and whether they align or show a gap; if either is missing, omit salary entirely — do not mention it, do not say it is missing; (5) LOCATION AND WORK TYPE — if the candidate's location differs from the job location, state both and say whether the candidate is open to relocation; if they already match, confirm briefly; if unknown, flag it; also mention the candidate's work preference (Remote/Hybrid/Onsite) and whether it is compatible with what the job requires — if the candidate's preference conflicts with the job's work arrangement, flag it; (6) close with the recommended next action. For Pass candidates: empty string.>"
}}
